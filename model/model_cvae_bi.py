    ...
    def train(self):
        ...
        for epoch in range(self.epoch_num):
            ...
            for two_samples in tqdm(train_loader, ncols=100):
                ...
                # iv_pred = iv_pred.view(len(iv_pred_list), -1, 22, 3)
                # bi_loss = self.loss_function(pred, iv_pred)
                ...
                # print((short_g_loss + long_g_loss + rv_g_loss))
                total_loss.backward()
                ...
                # writer.add_scalar("bi_loss", bi_loss, global_step=epoch * 317 + batch_idx)
        ...
        for batch_idx, two_samples in enumerate(test_loader):
            ...
            with torch.no_grad():
                ...
                if self.data_conf["save_metric"]:
                    ...
                    # contact ground detect
                    threshold = 2
                    foot_index = np.asarray(self.data_conf["foot_index"])
                    concat_pred_ = pred.view(pred.size(0), pred.size(1), -1)
                    foot_height = concat_pred_[:, :, foot_index[[1, 7]]]
                    ground_height_left = torch.min(foot_height[:, :, 0], dim=1) \
                        .values.unsqueeze(-1).repeat(1, pred.size(1))
                    ground_height_right = torch.min(foot_height[:, :, 1], dim=1) \
                        .values.unsqueeze(-1).repeat(1, pred.size(1))

                    update_height_left = foot_height[:, :, 0] - ground_height_left
                    update_height_right = foot_height[:, :, 1] - ground_height_right
                    new_foot_height = torch.cat([update_height_left.unsqueeze(-1), update_height_right.unsqueeze(-1)],
                                                2)
                    cal_contact_list = new_foot_height < threshold

                    pred_pos = pred.permute(0, 2, 3, 1)
                    ...
                    pred_vel = pred_vel.view(pred_vel.size(0), 2, 3, pred_vel.size(-1))
                    slide_loss = torch.mean(torch.abs(pred_vel *
                                                      cal_contact_list[:, :-1].permute(0, 2, 1).unsqueeze(2).cuda()))
                    print("slide loss:", slide_loss)
                    slide_list.append(slide_loss.cpu())
